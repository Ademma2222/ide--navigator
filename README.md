...
│   ├── src/
│   │   ├── extension.ts        — LSP-клиент, регистрация команд, host для WebView
│   │   └── webview-protocol.ts — типы сообщений между host и webview
│   ├── media/                  — разметка WebView-панелей
...
└── server/                     — Language Server (Python)
    ...
    ├── server.spec             — конфиг PyInstaller для standalone-сборки
    ├── requirements.txt
    ├── tests/                  — 48 unit-тестов pytest
    └── languages/
        ├── base.py             — фасад BaseLanguage, наследующий пять миксинов
        ├── _parse_cache.py     — ParseCacheMixin: tree-sitter + LRU + инкрементальный парсинг
        ├── _definition.py      — DefinitionMixin: go-to-def с workspace-sandbox
        ├── _references.py      — ReferencesMixin: find-references + CodeLens-счётчики
        ├── _hover.py           — HoverMixin: tooltip с complexity
        ├── _call_graph.py      — CallGraphMixin: call graph + McCabe + AST-хелперы
        ├── python_lang.py
        ├── java_lang.py
        ├── cpp_lang.py
        ├── go_lang.py
